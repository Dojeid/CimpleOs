// browser.cpp — Falkon Surf Web Browser App (C++)
extern "C" {
...
static void browser_load_page(const char* url) {
    ...
    if (strncmp(current_url, "file://", 7) == 0) {
        ...
        if (node && node->data) {
            ...
            sprintf(page_content, "404 Not Found: '%s' not on VFS.", path);
        }
    } else {
        sprintf(page_content,
            "Falkon Web Surf v1.0\n"
            "Fetching: %s\n"
            "Status: 200 OK | e1000 NIC Active\n",
            current_url);
    }
}
...
static void browser_redraw(window_t* win) {
    if (!win) return;
    int x = win->x + 8,  y = win->y + 32;
    int w = win->width - 16, h = win->height - 40;
    ...
    // Nav buttons
    draw_rect(x + 4,  y + 4, 24, 24, 0x334155); draw_string(x + 12, y + 9, 0xFFFFFF, "<");
    draw_rect(x + 32, y + 4, 24, 24, 0x334155); draw_string(x + 40, y + 9, 0xFFFFFF, ">");
    draw_rect(x + 60, y + 4, 24, 24, 0x334155); draw_string(x + 68, y + 9, 0xFFFFFF, "R");
    ...
    // Viewport
    int vp_y = y + 38, vp_h = h - 42;
    draw_rect(x, vp_y, w, vp_h, 0x0B0F19);
    draw_string(x + 12, vp_y + 12, 0x4ADE80, "[HTML DOM Viewport - Falkon Surf Renderer]");
    draw_rect(x + 12, vp_y + 26, w - 24, 1, 0x334155);
    draw_string(x + 12, vp_y + 36, 0xF1F5F9, page_content);
}
...
extern "C" void browser_open(const char* url) {
